...
ggplot(d1, aes(x= Treatment, y= Aerial_fresh_weight, fill= Species)) + 
  ...
  theme_pubclean()+
  labs(title = 'Boxplot 07. The effect of water deficit  on the aerial fresh weight (g) of the different species.')

```

> Beta vulgaris, Raphanus sativus and Sonchus oleraceus are clearly impacted by the two treatments in deficit of water. Solanum lycopersicum is and Hordeum vulgare are also impacted by mostly by the stress treatment (s). All those species are cultivated species except Sonchus oleraceus, and all would probably decrease in yield if they lack of water, according to this experiment.

### Aerial_dry_weight

> The Boxplot 08 shows the Aerial dry weight of thw different species, at the end of the experiement.
...
ggplot(d1, aes(x= Treatment, y= Aerial_dry_weight, fill= Species)) + 
  ...
  theme_pubclean()+
  labs(title = 'Boxplot 08. Effect of water deficit  on the aerial dry weight (cm) of the different species.')

```

> It is slightly less obvious in the Boxplot 08 that water scarcity affects the plants, but this analyse is not adapted to every species since they have very different morphologies. Nonetheless, R. sativus data correct with the previous analysis and its aerial dry weight is decreasing with the two treatements i and s. Other than that, Beta vulgaris, Hordeum vulgare, Solanum lycopersicum and Sonchus oleraceus are also showing a significant reduction of aerial dry weight by diminish the water availibility. We assume that Portulaca oleracea weight is too small to be interpreted. 

> The Boxplots 09, 10 and 11 will present the root measurements taken after the experiment. 
...
ggplot(d1, aes(x= Treatment, y= Root_length, fill= Species)) +
  ...
  theme_pubclean()+
  labs(title = 'Boxplot 09. Effect of water deficit  on the root lenght (cm) of the different species.')

  
```

> Boxplot 09 is showing that the water deficit during this short experiment still has an impact on the root lenght of some species (Hordeum vulgare and Raphanus sativus). Interestingly while Hordeum vulgare seems to reduce its roots lenght, Raphanus sativus will increase its root lenght under water scarcity. It is not significant enough for the other species that water stress has an effect on the root lenght, in this experiement.
...
ggplot(d1, aes(x= Treatment, y= Roots_fresh_weight, fill= Species)) + 
  ...
  theme_pubclean()+
  labs(title = 'Boxplot 10. The effect of water deficit  on the root fresh weight (g) of the different species.')
```
