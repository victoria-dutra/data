...
> In order to get a better understanding of the variability between weeks, the group chose to explore two main graphs for soil parameters, one with a cultivated species (Solanum lycopersicum) and one with a wild species (Lolium perenne). The following code chunk is performing the visual representation.
...
ggplot(s1, aes(x= Week, y= Soil_humidity, group= PlantId, color= Treatment)) + 
  geom_line() +
  labs(title = 'Figure 1: Evolution of the soil humidity of Solanum lycopersicum during the six weeks') +
  theme_pubclean() +
...
ggplot(s2, aes(x= Week, y= Soil_humidity, group= PlantId, color= Treatment)) + 
  geom_line() +
  labs(title = 'Figure 2: Evolution of the soil humidity of Lolium perenne during the six weeks') +
  theme_pubclean() +
  scale_color_viridis(discrete = TRUE)
```
...
ggplot(s3, aes(x= Week, y= Plant_height, group= PlantId, color= Treatment)) + 
  geom_line() +
  labs(title = 'Figure 3: Evolution of the plant height of Raphanus sativus during the six weeks') +
  theme_pubclean() +
  scale_color_viridis(discrete = TRUE)
```
